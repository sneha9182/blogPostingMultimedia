  ...
  return (
    <div className="bg-white-100 rounded-lg shadow border-black">
      <div className="space-y-3">
        {rejected.map((rejectedBlog, index) => {
          const firstLetter = rejectedBlog.user_name.charAt(0).toUpperCase();
          return (
            <div key={index} className="flex items-center p-4 bg-blue-100 last:border-b-0 shadow-md rounded-md">
              ...
              <div className="flex-1 gap-4 space-y-2">
                <div>
                  <p className="font-medium text-xl">{rejectedBlog.blog_title}</p>
                  <p className="font-small">{rejectedBlog.user_name}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500">{TimeAgo(rejectedBlog.blog_time)}</p>
                </div>
              </div>
              <div className="ml-auto">
                <button
                  onClick={() => handlePreview(rejectedBlog)}
